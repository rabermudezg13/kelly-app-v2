...
function HomePage() {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-sky-200 text-slate-900">
      {/* Desktop keeps the original full-screen treatment. On phones the image gets its own
          visible hero area so the school bus is never cropped out by a portrait viewport. */}
      <div
        className="absolute inset-0 hidden bg-cover bg-center sm:block"
        style={{ backgroundImage: "url('/welcome-aboard.jpg')" }}
      />
      <div
        className="absolute inset-x-0 top-0 h-[42vh] min-h-[300px] bg-contain bg-top bg-no-repeat sm:hidden"
        style={{ backgroundImage: "url('/welcome-aboard.jpg')" }}
      />
      <div className="absolute inset-0 bg-gradient-to-b from-white/5 via-white/10 to-sky-100/45 sm:bg-white/10" />
      ...
      <div className="relative z-10 mx-auto flex min-h-screen max-w-[1600px] flex-col px-4 py-4 sm:px-8 sm:py-5 lg:px-12">
        ...
        <main className="flex flex-1 flex-col items-center justify-center pb-6 pt-[25vh] sm:py-8">
          <section className="mb-5 w-full max-w-3xl rounded-[28px] border border-white/70 bg-white/65 px-5 py-5 text-center shadow-[0_24px_70px_rgba(15,23,42,0.18)] backdrop-blur-2xl sm:mb-7 sm:rounded-[34px] sm:px-10 sm:py-6">
            ...
            <p className="mt-3 text-sm font-semibold text-slate-700 sm:mt-4 sm:text-lg">People <span className="mx-2 text-green-600 sm:mx-3">•</span> Opportunity <span className="mx-2 text-green-600 sm:mx-3">•</span> Community</p>
          </section>

          {announcements.length > 0 && (
            <section className="mb-5 w-full max-w-4xl rounded-[22px] border border-white/60 bg-white/65 p-3 shadow-xl backdrop-blur-2xl sm:mb-6 sm:rounded-[24px] sm:p-4">
        ...
        <footer className="hidden flex-wrap items-center justify-between gap-3 pb-2 sm:flex">
